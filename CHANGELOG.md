...
- 本地记录冗余：客户端在累计 500 条写入、间隔到期或退出时创建通过校验的快照，覆盖历史数据库、加密图片对象与收藏文件束缓存。同一卷上优先使用硬链接，跨卷才复制。
- 启动自动恢复：历史数据库无法解密或已损坏时，先按时间从新到旧寻找通过校验的快照并恢复；只有不存在可用快照时，才备份原库并重建空库。
- 快照设置：可指定快照目录（默认 `%LOCALAPPDATA%\Clipboard\snapshots`，可指向另一块磁盘）、间隔（1–1440 分钟）与保留份数（1–10）。
- 「从远端恢复历史」入口：清空本地历史后，由重启的客户端从配置的远端拉取并合并历史。
- 新增 15（快照失败）与 16（快照恢复失败）号状态码，既有 0–14 保持不变。

### 测试

- Rust 覆盖快照创建与校验、拒绝为损坏库创建快照、对象进入快照、裁剪保留份数与清理残缺目录、被篡改快照被跳过。
- C ABI 覆盖创建、列举与恢复的完整往返，包括破坏活动库后恢复并用原密钥重新打开。
- 客户端覆盖快照设置校验、写入计数触发、快照目录规则与远端恢复的前置条件。

### 修复

- 快照目录必须是绝对路径且不得位于数据目录内部，避免快照与它要保护的数据库一起被删除。
- 创建快照前先以独立连接校验磁盘上的数据库，并拒绝为无法打开的库创建快照，避免把损坏内容当成备份。
...
- 新增损坏历史数据库备份重建的单元测试，覆盖正常备份、无数据库与文件名冲突三种情况。
...
- 修正剪贴板核心无法启动时应用完全不可见的问题。此前数据库打开失败会跳过托盘图标、全局热键与面板的创建，进程虽然存活，但用户看不到任何入口；现在托盘图标在任何可能失败的初始化之前创建，失败原因写入面板状态栏与日志。
- 新增损坏历史数据库的自动恢复。客户端遇到无法解密或已损坏的历史库时，先把原文件重命名为 `history.db.corrupt-<时间戳>` 保留备份，再用同一把密钥重建空库并继续启动。密钥不匹配不会被自动重建，避免误伤其他设备的数据。
...
[未发布]: https://github.com/KINNNNNNG/clipboard/compare/v0.1.2...HEAD
[0.1.2]: https://github.com/KINNNNNNG/clipboard/compare/v0.1.1...v0.1.2
