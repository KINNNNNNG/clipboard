...
> `[ ]` 仅保留给尚未完成的可见 WinUI、DPI 或人工系统验收；当前总状态见 [Clipboard 开发状态](../../STATUS.md)。
...
- [x] `pwsh -NoProfile -File scripts/verify-windows-client.ps1` 本机通过。
- [x] 文本和图片捕获在数据库提交后才刷新 UI，自身写回不会重复记录。
- [x] 图片对象以认证密文保存，篡改后失败关闭。
- [x] 搜索、正则、时间/来源/类型筛选和无效正则保留旧结果均通过。
- [x] 收藏、删除、全部清除和三个保留上限真实持久化，收藏项不自动清理。
- [x] Win+V 接管失败时备用快捷键保持应用可用。
- [ ] 面板在光标附近并约束于工作区，100/150/200% DPI 无重叠。
- [x] 只有确认原 HWND 恢复后才发送 Ctrl+V。
- [x] README 已将“阶段 2 不含文件历史和跨设备同步”的历史范围替换为当前阶段总览，并明确链接后续阶段能力与验收边界。
