...
- [x] **Step 3：实现只供性能使用的 fixture 二进制。**
...
在释放 Database 后只向标准输出写入一行 {"fixture":"history_search_performance","items":10000}。夹具不得读取用户剪贴板、用户数据库、远端配置或环境中的密钥。
...
- [x] **Step 3：实现串行 Release 脚本。**

脚本在 Cargo 返回后立即保存 `$LASTEXITCODE`，后续只在成功时继续；`finally` 中的目录恢复单独捕获。清理失败只输出固定安全类别，若 Cargo 已返回非零码则仍以该原始码退出；脚本不使用泛化 `throw` 覆盖子进程的诊断根因。

脚本不写结果文件，不调用默认 Windows 发布验证。

- [x] **Step 4：确认脚本门转绿并执行入口。**
...
- [x] **Step 1：写 Debug/Release 映射的失败测试。**
...
- [x] **Step 4：实现不含 UI 首帧声明的 JSON 诊断程序。**
...
先运行一轮不计时预热。FFI 子串和组合筛选各运行 30 个新 client 样本；ViewModel 样本从设置 QueryText 或 SetFilters 开始，通过 `PropertyChanged` 在 IsLoading 变为 false 且 Items.Count 正确时完成，必须包含 50 ms debounce、DTO 转换和 ObservableCollection 更新。空查询只运行 3 次并单独报告。每轮 client 与 ViewModel 都新建并释放，不宣称清除操作系统文件缓存。

图片诊断创建 new BoundedLruCache<Guid, byte[]>(64)，写入 65 个固定完整 1x1 PNG 的克隆字节数组，断言 Count == 64、第一个键不存在、最后一个键仍存在。记录该操作前后 Process.GetCurrentProcess().WorkingSet64。输出 JsonSerializer.Serialize 的对象，字段至少包含 git_revision、windows_version、dotnet_version、rust_version、logical_processor_count、physical_memory_bytes、configuration、warm_os_file_cache、cold_search_objects、ffi_substring、ffi_combined、view_model_substring、view_model_combined、empty_query、image_lru、peak_working_set_bytes、managed_heap_bytes。所有指标只记录数值，不记录测试文本、数据库目录、key、vault、用户名或远端信息。

- [x] **Step 5：实现临时目录 PowerShell 入口。**
...
脚本使用 [IO.Path]::GetTempPath() 和 [Guid]::NewGuid().ToString('N') 建立唯一目录，并按照下面顺序执行：
    ...
    target\release\prepare_history_performance.exe --data-dir $dataDirectory | Out-Null
...
使用 try/finally，以 Remove-Item -LiteralPath $dataDirectory -Recurse -Force 清理这个已验证的唯一临时子目录；清理异常以固定安全类别记录，且不得覆盖 Cargo、fixture、build 或程序的原始非零退出码。所有构建、工具链、fixture 和还原子进程输出均被捕获并抑制；诊断宿主输出先提取完整 JSON 对象，再经过严格字段类型和值域校验，成功时仅重写白名单报告，失败时仅输出固定安全类别。脚本不接受数据目录或 key 参数，不进入默认发布验证。

- [x] **Step 6：运行绿灯边界测试和完整诊断。**
...
- 日期：2026-08-25。
- git diff --check、cargo fmt --all --check、cargo clippy --workspace --all-targets -- -D warnings、cargo test --workspace --all-targets：均退出码 0；默认 Rust 测试中的历史搜索性能门保持 ignored，未执行样本。
- dotnet test tests/Clipboard.Windows.Tests/Clipboard.Windows.Tests.csproj -c Debug --no-restore --filter FullyQualifiedName~PerformanceEntrypointConfigurationTests：35/35 通过；契约覆盖 Debug/Release FFI 映射、独立入口隔离、清理后原始退出码、每个子进程输出捕获、递归重复 JSON 属性、严格字段类型和值域、脱敏失败输出、跨行尾测试片段读取、FFI 与 ViewModel 的超时释放、逐样本对象生命周期、事件驱动完成、冷搜索对象和完整 PNG LRU 输入。
- dotnet build tests/Clipboard.Windows.Performance/Clipboard.Windows.Performance.csproj -c Release --no-restore：退出码 0，0 个警告、0 个错误。
- pwsh -NoProfile -File scripts/measure-history-search.ps1：退出码 0；脚本仅输出三条经过严格校验的 JSON 指标，子串搜索 P95 29 ms，组合筛选 P95 29 ms，均低于 200 ms；空查询趋势 P95 72 ms。
- pwsh -NoProfile -File scripts/measure-windows-history-diagnostics.ps1：退出码 0；构建、工具链、fixture 和还原输出均被捕获并抑制，脚本仅输出一个经过严格校验的 JSON 报告，包含 warm_os_file_cache、cold_search_objects、FFI、ViewModel、空查询、图片 LRU、工作集和托管堆字段；FFI P95 为 30/22 ms，ViewModel P95 为 92/91 ms，空查询 P95 为 91 ms；LRU 保持 64 容量并淘汰最旧项。
- 注入子进程退出码 37 的端到端验证：两条性能脚本均返回 37；Windows 入口额外覆盖工具链、fixture 构建、fixture、还原和诊断宿主五个失败阶段，失败输出只保留固定安全类别且没有遗留新临时目录。
- pwsh -NoProfile -File scripts/verify-windows-client.ps1 -SkipGuiSmoke：退出码 0；定向 126/126、全量 231/231 Windows 测试通过，默认验证未启动 Clipboard.Windows.Performance。
