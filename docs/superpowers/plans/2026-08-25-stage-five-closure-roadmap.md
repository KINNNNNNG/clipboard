...
| P0 | 历史加载失败根因与恢复（本地已关闭，2026-09-11） | 可隔离的测试 vault | 锁定、损坏、迁移失败的故障注入与恢复结果 |
...
**状态：** 本地代码与自动证据已关闭，2026-09-11。设计、实现和测试证据见[历史加载失败分类实施计划](2026-09-11-history-load-failure-classification.md)；可见桌面上的真实 vault 人工启动流程仍属于发布验收。

**问题边界：** `ClipboardPanelViewModel` 曾在首次 `CoreError` 后延迟 75 ms 重试，并在再次失败时显示“无法加载剪贴板历史。”；现在锁、密钥不匹配、无法解密、损坏和迁移失败各自返回唯一 `CoreStatus` 和对应提示。

**根因：** SQLCipher 无法从错误码区分密钥错误和首页损坏，两者都返回 `NotADatabase`。因此新增非秘密 `history.vault` 标记记录数据库归属，在打开数据库之前判定 vault 不匹配，并把匹配标记下的 `NotADatabase` 判为损坏。

**证据：** 存储层 6 个 `vault_open` 故障注入、FFI 5 个状态码用例、Windows 定向 `132/132` 与全量 `240/240`。
...
- [x] 定义不会泄露路径、内容或密钥的错误分类，区分短暂 Core 调用失败、数据库锁、错误密钥、损坏数据库和迁移失败。
- [x] 为每个分类建立可重复的故障注入测试，断言保留已有历史、重试次数、用户提示和恢复后的重新加载行为。
- [x] 在隔离 vault 上执行启动、关闭、锁定、损坏和恢复流程，确认任何失败都不会覆盖原始数据或生成明文诊断。
- [x] 仅在根因可区分后调整 UI 提示、日志或恢复操作，并补充对应的 Windows 自动测试。
