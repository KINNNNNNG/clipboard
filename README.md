...
- 按累计写入、时间间隔或客户端退出自动创建经过校验的本地快照，覆盖历史数据库、加密图片对象与收藏文件束缓存。
- 数据库损坏或误删时优先从最近一个通过校验的快照恢复；只有不存在可用快照时，才备份原库并重建空库。
- 快照目录可指向另一块磁盘；同一磁盘上优先使用硬链接，几乎不占额外空间。
...
设置页“应用”分区可配置快照目录（默认 `%LOCALAPPDATA%\Clipboard\snapshots`，可指向另一块磁盘）、间隔（1–1440 分钟）与保留份数（1–10）。客户端在累计 500 条写入、间隔到期或退出时创建快照；快照只有通过解密校验与逐文件哈希比对后才会被认可，未通过校验的副本会被跳过，裁剪时同时回收不再被引用的对象与缓存文件。

“同步设置”分区提供“从远端恢复历史”：确认后清空本地历史，由重启的客户端从远端拉回并合并，原有本地数据库会备份保留。
